...
class Element115Tracker:
    ...
    def _ensure_synth(self):
        if self.synth is None:
            try:
                self.synth = synth.PatchSynth(num_voices=8, patch=self.patch)
            except Exception:
                self.synth = None
    ...
    def tick(self, now_ms):
        ...
        gate = max(40, int(step_ms * 0.68))
        for row in range(self.ROWS):
            if self.pattern[row][self.play_step]:
                note = self.notes[row]
                velocity = max(0.18, 0.46 - row * 0.025)
                if self.synth is not None:
                    try:
