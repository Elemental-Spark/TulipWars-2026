# TulipWars 2026 v0.1.8 complete install/update package.
# Upload this single file to Tulip Web/Desktop or Tulip CC, load it, and Run.
import os
import sys
import gc
import tulip
...
FILES = {'README.txt': 'TulipWars 2026 v0.1.8 FULL\n'
               '\n'
               ...
               '- Never allocates more than six managed Juno voices.\n'
               '- Ambient music uses four voices and cannot overlap a second ambient chord.\n'
               '- Opening Element115 Tracker completely releases the background PatchSynth.\n'
               '- Tracker playback is hard-capped to six simultaneous Juno notes.\n'
               '- Leaving Tracker releases its synth before procedural ambience is rebuilt.\n'
               '- Tulip Web exports now trigger a real browser .MID download.\n'
               '- Hardware/Desktop exports remain stored locally in the package folder.\n',
 'tulipwars2026.py': 'import sys\n'
                     ...
                     '        self._line(4, "CAPTAIN %-18s  SHIP %-20s" % (player.get("callsign", '
                     'self.local["callsign"]), player.get("ship_name", "LOST TULIP")), self.BRIGHT)\n'
                     ...
                     '                self._line(20 + index, "%-22s %s" % (captain.get("callsign", "ANON"), '
                     'captain.get("activity", "DOCKED")))\n'
                     '        else:\n'
                     ...
                     '            self._line(14 + index, "%-18s %3d" % (item.get("name", item.get("id", "ITEM")), '
                     'cargo.get(item.get("id"), 0)))\n'
                     '        self._footer()\n'
                     '\n'
                     '    def draw_navigation(self):\n'
                     '        self._header("NAVIGATION")\n'
                     '        player = self.data.get("player", {})\n'
                     '        self._line(4, "CURRENT SYSTEM: %s  //  FUEL: %s" % (player.get("system_name", '
                     '"UNKNOWN"), player.get("fuel", 0)), self.BRIGHT)\n'
                     '        self._line(6, "SELECT A DESTINATION. THE RELAY CALCULATES FUEL AND ENCOUNTERS.", '
                     'self.DIM)\n'
                     ...
                     '        player = self.data.get("player", {})\n'
                     '        self._line(4, "CREDITS: %s  //  SELECT ITEM, THEN BUY OR SELL ONE UNIT" % '
                     'player.get("credits", 0), self.BRIGHT)\n'
                     ...
                     '            self._line(4, "HOSTILE CONTACT: %s" % battle.get("enemy_name", "ALIEN"), '
                     'self.ALERT)\n'
                     '            self._line(6, "ENEMY HULL   %3d / %3d" % (battle.get("enemy_hull", 0), '
                     'battle.get("enemy_max_hull", 100)))\n'
                     ...
                     '            self._line(16, battle.get("log", "THE SIGNAL SCREAMS THROUGH THE VOID."), '
                     'self.ACCENT)\n'
                     ...
                     '        player = self.data.get("player", {})\n'
                     '        self._line(4, "LOCAL BOARD: %s // MANUAL POSTS ONLY" % player.get("system_name", '
                     '"UNKNOWN"), self.BRIGHT)\n'
                     ...
                     '            status = "BROWSER DOWNLOADED" if self.tracker.last_browser_download else "SAVED '
                     'LOCALLY"\n'
                     '            self._line(25, "LAST EXPORT: %s // %s" % (self.tracker.last_export, status), '
                     'self.GOOD)\n'
                     '        self._footer("[0] BACK   AMY AUDIO ONLY - NO SAMPLES")\n'
                     '\n'
                     '    def draw_world(self):\n'
                     '        self._header("TULIP WORLD PUBLIC LOUNGE")\n'
                     '        self._line(4, "READ-ONLY INSIDE TULIPWARS. NOTHING IS POSTED AUTOMATICALLY.", '
                     'self.GOOD)\n'
                     '        self._line(5, "USE run(\'worldui\') SEPARATELY WHEN YOU CHOOSE TO CHAT PUBLICLY.", '
                     'self.DIM)\n'
                     '        y = 7\n'
                     ...
                     '            command = "world.download(\'%s\',\'%s\')" % (tw_config.WORLD_PACKAGE, '
                     'tw_config.WORLD_AUTHOR)\n'
                     '        else:\n'
                     ...
                     '            if self.screen not in ("chat", "tracker") and not self.heartbeat_pending and not '
                     ...
                     '            tulip.tfb_str(0, y, text + (" " * max(0, 128 - len(text))), 0, 231 if y != 5 else '
                     '196, 0)\n'
                     '    except Exception:\n'
                     '        pass\n'
                     '\n'
 ...
 'tw_midi.py': '# Minimal Standard MIDI File type-0 exporter for the Element115 console.\n'
               ...
               '    events.append((0, -2, bytes([0xFF, 0x51, 0x03, (tempo >> 16) & 255, (tempo >> 8) & 255, tempo & '
               '255])))\n'
 ...
 'tw_net.py': 'import json\n'
              ...
              '\n'
              '        def failed(exc):\n'
              '            error = exc if isinstance(exc, NetworkError) else NetworkError("Connection failed: %s" % '
              'exc)\n'
              '            if fail is not None:\n'
              '                fail(error)\n'
              '                return None\n'
 ...
 'tw_tracker.py': 'import random\n'
                  ...
                  '                self.pattern[row][step] = (random.random() < density) or (row == 0 and anchor and '
                  'random.random() < 0.65)\n'
                  '        self.patch = random.choice(self.PATCHES)\n'
                  '\n'
                  '    def toggle(self):\n'
                  ...
                  '        self.last_export = tw_midi.export_midi(self.pattern, self.notes, self.bpm, filename, '
                  'self.patch)\n'
                  ...
                  '        return lines\n'}
OLD_USER_FILES = (
    'INSTALL_TULIPWARS_WEB.py',
    'INSTALL_TULIPWARS_WEB_v0.1.2.py',
    'INSTALL_TULIPWARS_WEB_v0.1.3.py',
    'INSTALL_TULIPWARS_WEB_v0.1.4.py',
    'INSTALL_TULIPWARS_WEB_v0.1.5.py',
    'INSTALL_TULIPWARS_WEB_v0.1.6.py',
    'INSTALL_TULIPWARS_WEB_v0.1.7.py',
    'INSTALL_TULIPWARS_WEB_v0.1.8.py',
    'INSTALL_TULIPWARS_v0.1.8.py',
    'INSTALL_OR_UPDATE_TULIPWARS_v0.1.8.py',
    'REPAIR_TULIPWARS_BLACK_SCREEN_v0.1.5.py',
    'tulipwars2026.tar',
)
PACKAGE_FILES = tuple(FILES.keys())


def _user_dir():
    try:
        return tulip.root_dir() + 'user'
    except Exception:
        return '/user'
...
def _clean_package(package_dir):
    # Preserve the anonymous local ship identity and every exported MIDI file.
    try:
...
def install():
    ...
    print('Hardware-safe six-Juno-voice cap + Web MIDI downloads')
